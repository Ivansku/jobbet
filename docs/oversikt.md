...
*Senast uppdaterad: 2026-08-18 (Anteckningsmallar)*
...
| `uppgiftstyp` | Fri, admin-hanterad lista med typer för att märka uppgifter (t.ex. "Möte", "Support"). `anteckningsmall_id` (nullable FK till `anteckningsmall`, "Visa anteckningar" i formuläret) styr om och med vilken anteckningsmall som standard uppgifter av typen visar anteckningssektionen. `skapa_uppgifter_vid_klar` (standard för auto-generering av uppföljningsuppgifter, se nedan) och `visar_mailinnehall` (visar ett fritt mailinnehålls-fält istället för blocksystemet) styrs också härifrån. |
