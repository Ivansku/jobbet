...
- `uppgift.status` has a CHECK constraint: allowed values are `oppen`, `pagar`, `kommande`, `vantar`, `oplanerad`, `klar` (ASCII, no diacritics — verified against `pg_get_constraintdef` 2026-09-18; the UI labels are the diacritic forms, but the stored/constraint values are not). Verify before inserting new values or ALTER the constraint first.
